...
void Compiler::optCloneLoop(FlowGraphNaturalLoop* loop, LoopCloneContext* context)
{
    ...
    FlowEdge* const edgeToHeader = preheader->GetTargetEdge();
    fgReplacePred(loop->GetHeader(), preheader, fastPreheader);
    assert(edgeToHeader->getSourceBlock() == fastPreheader);
    fastPreheader->SetTargetEdge(edgeToHeader);
    ...
    FlowEdge* const slowPreheaderEdge = fgAddRefPred(slowHeader, slowPreheader);
    slowPreheader->SetTargetEdge(slowPreheaderEdge);
    ...
    assert(preheader->KindIs(BBJ_ALWAYS));
    FlowEdge* const preheaderEdge = fgAddRefPred(preheader->Next(), preheader);
    preheader->SetTargetEdge(preheaderEdge);
