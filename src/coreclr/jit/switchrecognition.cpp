...
bool Compiler::optSwitchDetectAndConvert(BasicBlock* firstBlock)
{
    ...
    if (IsConstantTestCondBlock(firstBlock, &trueEdge, &falseEdge, &isReversed, &variableNode, &cns))
    {
        if (isReversed)
        {
        ...
        for (const BasicBlock* currBb = firstBlock->Next(); currBb != nullptr; currBb = currBb->Next())
        {
            ...
            if (IsConstantTestCondBlock(currBb, &currTrueEdge, &currFalseEdge, &isReversed, &currVariableNode,
                                        &currCns))
            {
                if (currTrueEdge->getDestinationBlock() != trueEdge->getDestinationBlock())
                {
